...
A **time series** is a sequence of data points collected or recorded at specific time intervals (e.g., daily stock prices, monthly rainfall). The goal of time series analysis is to understand the underlying patterns—such as trends and seasonality—to predict future values.

### Key Concept: Stationarity

For an ARIMA model to work, the data must be **stationary**. This means its statistical properties (mean and variance) do not change over time.

- **Trend**: A long-term increase or decrease in the data.
- **Seasonality**: A repeating pattern related to the calendar (e.g., higher sales every December).
- **Variance**: The "spread" of the data; if it grows or shrinks over time, the data is non-stationary.

#### Which of these series are stationary? 

a. Google closing stock price in 2015; 
b. Daily change in the Google stock price in 2015; 
...
h. Quarterly Australian beer production; 
i. Monthly Australian gas production.
...
- We use quantmod to fetch real-world data from Yahoo Finance. The xts and zoo packages ensure R treats the index as a "Date" rather than just a row number.
 

```{r Fetch the MSFT stock data}
...
a. **Visual inspection with line plots**  
   Plot the variable against time to see trends, seasonality, cycles, and abrupt changes.

b. **Seasonal decomposition**  
   Use tools like `stl()` or `decompose()` to break the series into trend, seasonal, and residual components.

c. **Check stationarity**  
   - Visual check: rolling mean and variance.  
   - Statistical tests: Augmented Dickey-Fuller (ADF) or KPSS test.

d. **Distribution analysis**  
   Histograms, boxplots (by month/quarter) to understand the overall distribution and detect outliers.

e. **Lagged analysis**  
   - **Autocorrelation Function (ACF)** and **Partial Autocorrelation Function (PACF)** plots to identify AR/MA orders.  
   - Scatter plots of series vs. its lags.

f. **Summary statistics by time segments**  
   For example, mean and variance per month or year to spot heteroscedasticity or seasonal effects.

g. **Check for missing values and outliers**  
...
```{r}
# Decompose stock close price data and see the pattern 
stock_as_ts <- ts(as.numeric(stockData$close), frequency = 252) 
decomp <- stl(stock_as_ts, s.window = "periodic") 
plot(decomp, main = "Seasonal Decomposition of close Price of MSFT") 
```



- Step 3: **Check stationarity**
- Forecasting is only reliable when data is stationary (consistent mean and variance over time).
...
```{r}
roll_mean <- rollmean(stockData$close, k = 5, fill = NA)
roll_std <- rollapply(stockData$close, width = 5, FUN = sd, fill = NA)

plot(AirPassengers, main = "Rolling Mean & Std Dev")
lines(roll_mean, col = "red", lwd = 2)
lines(roll_std, col = "green", lwd = 2)

# As the original stock data is non-stationary, checking to see if lagged differences are stationary 

lagged_stockData <- diff(stockData$close, lag = 1)

lagged_stockData <- na.omit(lagged_stockData)

adf_test <- adf.test(lagged_stockData) 
...
- Histograms and boxplots help detect outliers and understand the price spread
...
- ACF and PACF plots are essential for identifying the $p$ (AR) and $q$ (MA) orders for ARIMA models.
...
# this step to check if the data has missing value
#for example:
var1 <- ts(c(4, 5, 8, NA, 4, NA), 1) 
#the code is:
sum(is.na(var1)) 
...
The Box-Cox transformation uses a parameter called **Lambda ($\lambda$)** :
...
The magic is that the `BoxCox.lambda()` function in the below code automatically finds the perfect value for lambda to be used for the specific dataset.
```{r}
...
```{r Run acf on residuals}
model <- auto.arima(stockData$close) 
summary(model) 
```
```{r}
acf(model$residuals)
...
predicted_price <- forecast(model, 10)
